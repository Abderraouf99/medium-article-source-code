...
def clean_folder(file_formats: dict, curr_directory: str):
    logger.log(f"Starting to clean '{curr_directory}' folder")

    dir_content = os.listdir(curr_directory)
    ...
    for d_file in dir_content:
        try:
            ...
            target_path = f'{curr_directory}/{file_formats[file_extension.lower()]}'

            if not os.path.isdir(target_path):
                os.mkdir(target_path)

            src_path = f'{curr_directory}/{d_file}'
    ...
    logger.log(f"Finished cleaned '{curr_directory}' folder")
...
if __name__ == '__main__':
    ...
    if len(file_formats) == 0:
        ...
        clean_folder(file_formats=file_formats, curr_directory=curr_directory)
